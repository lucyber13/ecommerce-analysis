...
def plot_line_trend(comp_acc_df):
    fig = go.Figure()
    colors = {'computers': '#14b8a6', 'computers_accessories': '#f59e0b'}
    ...
    for col in [c for c in ['computers', 'computers_accessories'] if c in comp_acc_df.columns]:
        fig.add_trace(go.Scatter(
            ...
            line=dict(color=colors.get(col, '#818cf8'), width=2.5),
            marker=dict(size=5, color=colors.get(col, '#818cf8')),
            fill='tozeroy',
            fillcolor=colors.get(col, '#818cf8').replace('#', 'rgba(') + ',0.06)' if col == 'computers' else 'rgba(245,158,11,0.06)',
        ))
...
def plot_corr_heatmap(corr_matrix):
    ...
    fig.update_layout(**pl(
        ...
        height=480,
        xaxis=dict(tickfont=dict(size=10), tickangle=-30, gridcolor='transparent'),
        yaxis=dict(tickfont=dict(size=10), gridcolor='transparent'),
    ))
    return fig
